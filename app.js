...
function buildDraftInputs() {
  ...
  for (const side of ["A", "B"]) {
    ...
    for (const role of ROLES) {
      ...
      input.className = "draft-pick-input";
      input.placeholder = "Sem pick";
      input.autocomplete = "off";
      input.spellcheck = false;

      const list = document.createElement("div");
      list.className = "draft-autocomplete-list";
      list.hidden = true;
      ...
      initPickAutocomplete(input, list);

      wrap.append(input, list, meta);
...
function initAutocomplete(input, list, getItems) {
  let activeIndex = -1;

  function renderList(items) {
    list.innerHTML = "";
    activeIndex = -1;
    const selectable = (items || []).filter((entry) => typeof entry === "string");
    if (!selectable.length) { list.hidden = true; return; }
    for (const entry of items) {
      if (typeof entry !== "string") {
        const header = document.createElement("div");
        header.className = "draft-autocomplete-header";
        header.textContent = entry.header;
        list.appendChild(header);
        continue;
      }
      const item = document.createElement("div");
      item.className = "draft-autocomplete-item";
      item.textContent = entry;
      item.addEventListener("mousedown", (e) => { e.preventDefault(); selectValue(entry); });
  ...
  function openList() { renderList(getItems(input.value.trim())); }

  function closeList() { list.hidden = true; activeIndex = -1; }

  function highlightItem(index) {
    const items = list.querySelectorAll(".draft-autocomplete-item");
  ...
  input.addEventListener("input", () => {
    if (!input.value.trim()) updatePrediction();
    openList();
  });
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (list.hidden || !items.length) {
      if (e.key === "Enter") { e.preventDefault(); updatePrediction(); }
      return;
    }
    if (e.key === "ArrowDown") {
      e.preventDefault();
      ...
      e.preventDefault();
      if (activeIndex >= 0) selectValue(items[activeIndex].textContent);
      else if (items.length === 1) selectValue(items[0].textContent);
    } else if (e.key === "Escape") {
      closeList();
    }
  });

  // impede que clique na barra de rolagem da lista tire o foco do input
...
function championListForRole(role) {
  const all = [...state.indexes.champions];
  const inRole = all.filter((c) => championRoleSample(c, role) > 0).sort((a, b) => a.localeCompare(b));
  const inRoleSet = new Set(inRole);
  const others = all.filter((c) => !inRoleSet.has(c)).sort((a, b) => a.localeCompare(b));
  const items = [];
  if (inRole.length) items.push({ header: role }, ...inRole);
  if (others.length) items.push({ header: "Outros" }, ...others);
  return items;
}

function initPickAutocomplete(input, list) {
  initAutocomplete(input, list, (query) => {
    const role = input.dataset.role;
    if (!query) return championListForRole(role);
    const q = query.toLowerCase();
    const all = sortedChampionsForRole(role);
    const prefix = all.filter((c) => c.toLowerCase().startsWith(q));
    const rest   = all.filter((c) => !c.toLowerCase().startsWith(q) && c.toLowerCase().includes(q));
    return [...prefix, ...rest].slice(0, 8);
  });
}

function teamListForLeague(league) {
  const inLeague = [...(state.indexes.teamsByLeague.get(league) || [])].sort((a, b) => a.localeCompare(b));
  const inSet = new Set(inLeague);
  const othersSet = new Set();
  for (const [lg, teams] of state.indexes.teamsByLeague) {
    if (lg === league) continue;
    for (const team of teams) if (!inSet.has(team)) othersSet.add(team);
  }
  const others = [...othersSet].sort((a, b) => a.localeCompare(b));
  return { inLeague, others };
}

function initTeamAutocomplete(input, list) {
  initAutocomplete(input, list, (query) => {
    const league = state.elements.leagueSelect.value;
    const { inLeague, others } = teamListForLeague(league);
    if (!query) {
      const items = [];
      if (inLeague.length) items.push({ header: leagueLabel(league) }, ...inLeague);
      if (others.length) items.push({ header: "Outras ligas" }, ...others);
      return items;
    }
    const q = query.toLowerCase();
    const pool = [...inLeague, ...others];
    const prefix = pool.filter((t) => t.toLowerCase().startsWith(q));
    const rest   = pool.filter((t) => !t.toLowerCase().startsWith(q) && t.toLowerCase().includes(q));
...
function resolveTeamName(value) {
  ...
  if (!typed) return "";
  const all = new Set();
  for (const teams of state.indexes.teamsByLeague.values()) for (const t of teams) all.add(t);
  if (all.has(typed)) return typed;
  const lower = typed.toLowerCase();
  for (const t of all) if (t.toLowerCase() === lower) return t;
  const prefix = [...all].filter((t) => t.toLowerCase().startsWith(lower));
...
function refreshDraftPickOptions() {
  for (const el of document.querySelectorAll("[data-pick-slot]")) {
    if (el.tagName === "INPUT") continue; // text inputs filtragem é dinâmica, sem opções a rebuildar
    const selected = el.value;
    const role = el.dataset.role || roleFromSlot(el.dataset.pickSlot);
    el.innerHTML = "";
    el.appendChild(option("", "Sem pick"));
    for (const champ of sortedChampionsForRole(role)) {
      el.appendChild(option(champ));
    }
    if (selected && [...el.options].some((item) => item.value === selected)) {
      el.value = selected;
    }
  }
}
...
function updatePatchAndTeams() {
  ...
  const prevA = state.elements.teamASelect.value;
  const prevB = state.elements.teamBSelect.value;
  state.elements.teamASelect.value = teams.includes(prevA) ? prevA : (teams[0] || "");
  const keepB = teams.includes(prevB) && prevB !== state.elements.teamASelect.value;
  state.elements.teamBSelect.value = keepB
    ? prevB
    : (teams.find((team) => team !== state.elements.teamASelect.value) || "");
...
function updateTimingAlerts(pickCount, evOverValue, evUnderValue, draftMarket) {
  ...
  if (p8LabelEl) {
    if (isPremium) {
      const confluencePremiumOver = draftMarket?.confluence?.premium && draftMarket?.side === "over";
      p8LabelEl.textContent = confluencePremiumOver
        ? "★ SINAL PREMIUM — histórico 19/19, quase sempre OVER"
        : `★ SINAL PREMIUM — apostar ${bestSide.toUpperCase()}`;
    } else {
      p8LabelEl.textContent = "Pick 8 — ponto ótimo de precisão";
    }
  }
...
function updatePrediction() {
  ...
  const baseLabel = league === WORLD_LEAGUE ? "Base mundial" : "Base liga";
  state.elements.leagueBaseHint.textContent = `${baseLabel} ${round(leagueMean, 2)}\n${leagueGames.length} mapas`;
  ...
  updateTimingAlerts(selectedPicks.length, evOver.ev, evUnder.ev, draftMarket);
...
function clearPicks() {
  ...
  for (const list of document.querySelectorAll(".draft-autocomplete-list")) {
    list.hidden = true;
  }
  updatePrediction();
}
...
function init() {
  ...
  state.games = (data.games || []).filter((game) => DATA_LEAGUES.includes(game.league) && Number.isFinite(game.totalKills));
  ...
  initTeamAutocomplete(state.elements.teamASelect, getEl("teamAList"));
  initTeamAutocomplete(state.elements.teamBSelect, getEl("teamBList"));
  for (const el of [state.elements.teamASelect, state.elements.teamBSelect]) {
    el.addEventListener("change", () => {
      el.value = resolveTeamName(el.value);
      updatePrediction();
    });
  }
