...
function initPickAutocomplete(input, list) {
  ...
  function filterChampions(query) {
    ...
    const all = sortedChampionsForRole(input.dataset.role);
  ...
  function renderList(items) {
    ...
    for (const champ of items) {
      ...
      item.textContent = champ;
      item.addEventListener("mousedown", (e) => { e.preventDefault(); selectChamp(champ); });
  ...
  input.addEventListener("input", () => {
    ...
    if (!q) { closeList(); updatePrediction(); return; }
    renderList(filterChampions(q));
  });
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      if (activeIndex >= 0) selectChamp(items[activeIndex].textContent);
      else if (items.length === 1) selectChamp(items[0].textContent);
  ...
  input.addEventListener("focus", () => {
    const q = input.value.trim();
    if (q) renderList(filterChampions(q));
  });
}
